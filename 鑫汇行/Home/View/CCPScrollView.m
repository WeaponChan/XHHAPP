...
- (void)setTitleArray:(NSArray *)titleArray {
    ...
    [_titleNewArray removeAllObjects];
    ...
    for (int i = 0; i < objArray.count; i++) {
        ...
        titleLabel.frame = CGRectMake(labelX +10, labelY, labelW-20, labelH);
        
//        titleLabel.text = objArray[i];
        
        NSArray *arr = objArray[i];
        for (int j = 0; j<arr.count; j++) {
            UILabel *lab = [[UILabel alloc] initWithFrame:CGRectMake(0, j*30, titleLabel.frame.size.width, 25)];
            lab.textAlignment = NSTextAlignmentCenter;
            lab.textColor = RGBA(103, 103, 103, 1);
            lab.font = [UIFont systemFontOfSize:15];
            [lab setAttributedText:[self setAttributeStr:objArray[i][j]]];
            [titleLabel addSubview:lab];
        }
