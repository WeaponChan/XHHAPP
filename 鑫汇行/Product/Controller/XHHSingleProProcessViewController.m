//
//  XHHSingleProProcessViewController.m
//  鑫汇行
//
...
@interface XHHSingleProProcessViewController ()<UIWebViewDelegate>{
    NSString *pro_id;
}
...
@implementation XHHSingleProProcessViewController
...
-(void)setupView{
    ...
    _webView = ({
        UIWebView *webView = [[UIWebView alloc]initWithFrame:CGRectMake(0, 45, self.view.bounds.size.width, self.view.bounds.size.height-99)];
    ...
    [LhkhHttpsManager requestWithURLString:url parameters:nil type:1 success:^(id responseObject) {
        NSString *url = responseObject[@"list"][@"pro_process"];
