...
@implementation UIImage (LhkhExtension)
//改变图片颜色
...
-(UIImage *)imageWithwatermark:(UIImage*)image watermark:(NSString *)watermark{
    ...
    CGSize titleSize = [watermark sizeWithFont:[UIFont systemFontOfSize:16] constrainedToSize:CGSizeMake(MAXFLOAT, 30)];
//    NSAttributedString *waterAttr = [[NSAttributedString alloc]initWithString:watermark ];
//    
//    CGSize titleSize = [waterAttr boundingRectWithSize:CGSizeMake(MAXFLOAT, 30) options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin  context:nil].size;
    NSLog(@"---->%@",NSStringFromCGSize(titleSize));
    [watermark drawAtPoint:CGPointMake(image.size.width -titleSize.width - 10 , image.size.height - titleSize.height - 130) withAttributes:dic];
